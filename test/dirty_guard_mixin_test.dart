          ...
          child: Builder(
            builder: (context) => TextButton(
              onPressed: () => Navigator.of(context).push(
                MaterialPageRoute(builder: (_) => const _GuardedPage()),
              ),
...
class _DraftPageState extends State<_DraftPage>
  ...
  void initState() {
    super.initState();
    attachDraft(widget.store, 'thing');
  }
...
void main() {
  testWidgets(
    'confirming discard clears dirty and pops the route',
    (tester) async {
      ...
      expect(find.byType(_GuardedPage), findsNothing,
  ...
  testWidgets(
    'markDirtyAndPersist debounces and persists after 500ms',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final store = FormDraftStore(ttl: const Duration(hours: 24));
  ...
  testWidgets(
    'maybeRestoreDraft restores when a fresh draft exists',
    (tester) async {
      // Pre-seed a draft.
      SharedPreferences.setMockInitialValues({});
      final store = FormDraftStore(ttl: const Duration(hours: 24));
      ...
      await tester.pumpAndSettle();
      expect(find.text('恢复未完成的事项？'), findsOneWidget);
  ...
  testWidgets(
    'maybeRestoreDraft discarding clears the draft',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final store = FormDraftStore(ttl: const Duration(hours: 24));
      await store.save('thing', {'value': 'persisted'});
  ...
  testWidgets(
    'clearDraft removes the persisted draft after save',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final store = FormDraftStore(ttl: const Duration(hours: 24));
